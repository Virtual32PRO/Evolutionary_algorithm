    ...
    for k in range(m):
        ...
        for index, slownik in enumerate(podzial):
            ...
            for key, adres in slownik.items():
                i, j = adres
                if 0 <= i < len(macierz) and 0 <= j < len(macierz[0]) and (
                        macierz[i][j] == 1 or macierz[i + 6][j] == 1):
                    ids.append(key)
            matrix.append(ids)
        all.append(matrix)
    return all
...
def random_verse(m):
    ...
    if (bad_address[0] in address and bad_address[1] in address) or (
            (good_address[0] in address) ^ (good_address[1] in address)):
        address = sample(range(m), 3)
    for i in address:
        verse[i] = 1
    return verse
